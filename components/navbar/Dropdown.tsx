...
const Dropdown: NextPage<Props> = ({ data }) => {
  return (
    <div
      className={`group last:mr-0 cursor-pointer hover:text-gray-900 :hover:text-white transition-all duration-200 flex justify-center`}
    >
      <span>{data.name}</span>
        ...
        <ul className="bg-white p-2 rounded-md">
          {data.options.map((val) => (
            ...
            >
              <Link
                href={val?.href}
                className={` last:mr-0 cursor-pointer hover:text-gray-900 :hover:text-white transition-all duration-200`}
              >
                {" "}
                - {val?.name}
              </Link>
